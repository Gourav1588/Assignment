...
def make_payload(job_id: str, **kwargs) -> CandidateCreate:
    defaults = {
        "first_name": "Rahul",
        "last_name": "Sharma",
...
async def test_create_candidate_success():
    ...
    candidate = await service.create_candidate(
        make_payload(str(job.id)), created_by="hr@nucleusteq.com"
    )
...
async def test_create_candidate_duplicate_mobile():
    ...
    with pytest.raises(ConflictException):
        await service.create_candidate(
            make_payload(str(job.id), email="other@gmail.com"),
            created_by="hr@nucleusteq.com"
        )
...
async def test_create_candidate_invalid_job():
    ...
    with pytest.raises(ResourceNotFoundException):
        await service.create_candidate(
            make_payload("000000000000000000000000"),
            created_by="hr@nucleusteq.com"
        )
